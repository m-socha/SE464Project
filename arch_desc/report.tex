  ...
  \subsection{Android Application}
    ...
    a service is started through a call to startRequest(), any existing service is cancelled. On the other hand, MultiApiServices may have multiple requests active,
    and a request is only cancelled in calls to startRequest() if it shares the requestId of the new request. Concrete ApiRequest classes extend from either
