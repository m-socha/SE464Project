...
public class NotebookNotesService extends SingleApiService {
    public interface SearchNoteCompletionCallback {
        void onSuccess(List<Note> notes);
    }

    private SearchNoteService.SearchNoteCompletionCallback mCompletionCallback;
    private DefaultFailureCallback mFailureCallback;

    public void requestService(int notebookId,
                               SearchNoteService.SearchNoteCompletionCallback completionCallback,
                               DefaultFailureCallback failureCallback) {
        mCompletionCallback = completionCallback;
        mFailureCallback = failureCallback;

        ApiRequest request = new ApiRequest("notebooks/" + notebookId + "/notes", ApiRequest.RequestType.GET, this);
